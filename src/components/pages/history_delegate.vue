    ...
    <div class="content-body has-header">
      <div class="main-bg" id="init">
        <mt-loadmore class="main-bg" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore" :auto-fill="false" >
          ...
          <div v-if="positions.length==0" class="no-data main-text-color2">暂无历史委托</div>
        </mt-loadmore>
      </div>
    </div>
...
export default {
  ...
  created(){
    var objs = {
      "Ts":this.$store.state.delegateDate.endDate,
      "Te":this.$store.state.delegateDate.startDate,
  ...
  methods: {
    back () {
      ...
      this.$store.commit("CHANGE_DELEGATE_DATE",obj)
    ...
    get_data(obj){
      return  this.$http.post(Api.historyCommDay,obj).then(res=>{
    ...
    //打开日历
    arouseStart () {
      this.$router.push({path: '/calendar', query: {origin: 'history-delegate', start: this.$store.state.delegateDate.startDate, end: this.$store.state.delegateDate.endDate}});
    },
    //打开日历
    arouseEnd () {
      this.$router.push({path: '/calendar', query: {origin: 'history-delegate', start: this.$store.state.delegateDate.startDate, end: this.$store.state.delegateDate.endDate}});
  ...
  mounted () {
    this.startDateStr = this.$store.state.delegateDate.start;
    this.endDateStr = this.$store.state.delegateDate.end;
  },
