<template>
  <div class="withdraw_records">
    <CHeader title="提现记录" backBol="true" backUrl="/withdraw"></CHeader>
    <div class="font28 content-body has-header main-bg">
      <div class="time-filter normal-padding main-bg normal-border-color">
        ...
        <div class="assist-color3 date-piker-wrap">
          <input type="text" @focus="arouseStart" v-model="startDateStr" placeholder="起始日期">
          ...
          <span>~</span>
          <input type="text" @focus="arouseEnd" v-model="endDateStr" placeholder="截止日期">
          ...
          <mt-loadmore :bottom-method="loadBottom" ref="loadmore" :bottom-all-loaded="isAllLoaded" :auto-fill="false">
              <div v-if="recordsList.length" class="load-body">
                <div v-for="(item,index) in recordsList" :key="index">
                  <!-- <div class="assist-color3 main-text-color2 month-total normal-padding">{{item.Ym}}<span>提现：{{subMoney(item.GetMoneys)}}元</span></div> -->
                  <ul>
                    <router-link :to="{path: '/withdraw-details', query: {id: zitem.Id} }" tag="li" class="normal-border-color" v-for="zitem in item.MoneyFlowList" :key="zitem.Id">
                      <p>提现<span>({{zitem.strName}})</span></p>
...
export default {
  ...
  methods: {
    arouseStart () {
      this.$router.push({path: '/calendar', query: {origin: 'withdraw-records', start: this.$store.state.withdrawDate.startDate, end: this.$store.state.withdrawDate.endDate}});
    ...
    getRecordList (page) {//获取提现记录
      if (this.login) {
        return this.$http.post(api.withList, {
          "strTime": this.$store.state.withdrawDate.startDate + '',
          "enTime": this.$store.state.withdrawDate.endDate + '',
          "page": page,
          "limit": this.limit
        }).then(res => {
          let state = true;
  ...
  mounted () {
    this.startDateStr = this.$store.state.withdrawDate.start;
    this.endDateStr = this.$store.state.withdrawDate.end;
  },
