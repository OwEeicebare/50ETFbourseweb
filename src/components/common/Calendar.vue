<template>
  <div class="calendar normal-padding font28">
    <div class="rise-color month font32 normal-margin">{{month === 1 ? year : ''}}<span v-if="month === 1">&nbsp;&nbsp;</span>{{monthText}}</div>
    <ul>
      <li>日</li>
      <li>一</li>
      <li>二</li>
    ...
    <ul>
      ...
      <li v-for="(item,index) in days" :key="index" :class="{'selected': item == start || item == end, 'section': item > start && item < end && !theme, 'section-black': item > start && item < end && theme, 'normal-margin': true, 'has-start': end > start && item == start && !theme,'has-start-black': end > start && item == start && theme, 'has-end': end > start && item == end && !theme, 'has-end-black': end > start && item == end && theme}" @click="$emit('ensureStartOrEnd', {date: item, year: year, month: month})" ><span>{{item}}</span></li>
    </ul>
  </div>
</template>
<script>
...
<style scoped>
  ...
  .calendar > ul > li.section-black{
    background: rgba(255, 120, 123, 0.5);
  }
  ...
  .calendar > ul > li.has-start-black:after{
    position: absolute;
    width: 50%;
    height: 100%;
    right: 0;
    top: 0;
    background-color: rgba(255, 120, 123, 0.5);
    content: '';
  }
  ...
  .calendar > ul > li.has-end-black:after{
    position: absolute;
    width: 50%;
    height: 100%;
    left: 0;
